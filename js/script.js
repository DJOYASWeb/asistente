...
// Crear referencia a Firestore (usa tu config de firebase.js)
const db = firebase.firestore();

// Abrir el Modal
function abrirModalBlog() {
  document.getElementById('modalCrearBlog').classList.add('show');
}

// Cerrar el Modal
function cerrarModalBlog() {
  document.getElementById('modalCrearBlog').classList.remove('show');
}

// Guardar entrada en Firestore
async function guardarEntradaBlog(event) {
  event.preventDefault();

  const id = document.getElementById('blogId').value.trim();
  const nombre = document.getElementById('blogNombre').value.trim();
  const fecha = document.getElementById('blogFecha').value;
  ...
  try {
    await db.collection("blogsManual").doc(id).set({
      id,
      nombre,
      fecha,
      contenido,
    ...
    cerrarModalBlog();
    mostrarPopup(); // reutilizamos tu popup iOS de éxito
    cargarEntradasBlogs(); // refrescamos la lista
  } catch (error) {
...
// Cargar entradas creadas
async function cargarEntradasBlogs() {
  const contenedor = document.getElementById('blogList');
  contenedor.innerHTML = "<p class='text-muted'>Cargando...</p>";

  try {
    const snapshot = await db.collection("blogsManual").orderBy('creado', 'desc').get();
    contenedor.innerHTML = "";

    if (snapshot.empty) {
      contenedor.innerHTML = "<p class='text-muted'>No hay entradas aún.</p>";
      return;
    }
    ...
    snapshot.forEach(doc => {
      ...
      card.innerHTML = `
        <h6>${data.nombre}</h6>
        <p class="text-muted">Fecha: ${data.fecha}</p>
        <p>${data.contenido.substring(0, 100)}...</p>
        <button class="btn btn-outline-primary btn-sm mt-2" onclick="eliminarEntradaBlog('${data.id}')">🗑 Eliminar</button>
      `;
      contenedor.appendChild(card);
    });
  } catch (error) {
...
async function eliminarEntradaBlog(id) {
  ...
  try {
    await db.collection("blogsManual").doc(id).delete();
    mostrarPopup();
    cargarEntradasBlogs();
  } catch (error) {

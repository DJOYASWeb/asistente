...
const descripcionTexto = (descripcionRaw || "").toString().replace(/<[^>]+>/g, " ").replace(/\s+/g, " ").trim();
...
function prepararModal() {
  const thead = document.getElementById("columnasFinalesHead");
  const tbody = document.getElementById("columnasFinalesBody");
  ...
  const limpiar = (s) => s
    ...
    .replace(/\s+/g, " ")
    .trim();
  const safe = (s) => limpiar(s).replace(/"/g, "&quot;");
  ...
  if (tipoSeleccionado === "combinacion_cantidades" && datosCombinacionCantidades.length) {
    ...
    thead.innerHTML = columnas.map(c => `<th class="text-nowrap small">${c}</th>`).join("");
    tbody.innerHTML = datosCombinacionCantidades.map(fila => `
      <tr style="height:36px;">
        ${columnas.map(col => {
          const v = limpiar((fila[col] ?? "").toString());
          const preview = v.length > 40 ? v.slice(0, 40) + "…" : v;
          return `<td class="small" style="max-width:160px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;" title="${safe((fila[col] ?? "").toString())}">${preview}</td>`;
        }).join("")}
      </tr>`).join("");
  ...
  thead.innerHTML = columnas.map(c => `<th class="text-nowrap small">${c}</th>`).join("");
  tbody.innerHTML = transformados.map(fila => `
    <tr style="height:36px;">
      ${columnas.map(col => {
        const v = limpiar((fila[col] ?? "").toString());
        const preview = v.length > 40 ? v.slice(0, 40) + "…" : v;
